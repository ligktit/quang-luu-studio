...
def _make_mute_callback(dashboard, cc_key, mute_cc_map):
    def toggle(is_muted):
        ...
        try:
            ...
            print(f"⚠️ [MUTE MULTI] Lỗi gửi CC phụ cho {cc_key}: {e}")
    return toggle
...
def build_panel_mixer(dashboard) -> GlassPanel:
    ...
    for ch in channels:
        ...
        def _bind_mute(cc_key=ch["cc"]):
            def _do_toggle():
                curr = not dashboard.mute_states.get(cc_key, False)
                _make_mute_callback(dashboard, cc_key, mute_cc_map)(curr)
            return _do_toggle
